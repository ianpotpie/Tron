...
def alpha_beta_custom_voronoi(asp, cutoff_ply, weights):
    ...
    for action in asp.get_available_actions(start_state):
        ...
        next_score = cutoff_ab_min_value(asp, next_state, player, alpha, beta, cutoff_ply - 1, weights)
...
def cutoff_ab_max_value(asp, state, player, alpha, beta, depth, weights):
    ...
    if asp.is_terminal_state(state):
        ...
        return voronoi_v3(state,player,weights)
    else:
        max_score = float("-inf")
        for action in asp.get_available_actions(state):
            next_state = asp.transition(state, action)
            next_score = cutoff_ab_min_value(asp, next_state, player, alpha, beta, depth - 1, weights)
...
def cutoff_ab_min_value(asp, state, player, alpha, beta, depth, weights):
    """Inputs: asp - an adversarial search problem
           state - the current game state
           player - the player whose turn it is
